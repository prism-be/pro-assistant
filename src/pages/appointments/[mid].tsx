...
const Appointments: NextPage = () => {
  ...
  const { data: appointment, mutate: mutateAppointment } =
    useSWR<Appointment | null>(
      "/data/appointments/" + router.query.mid,
      loadAppointment
    );
  ...
  function computeDate() {
    ...
    if (newHour.getFullYear()) {
      if (
        newHour.getHours() === date.getHours() &&
        newHour.getMinutes() === date.getMinutes()
      ) {
        return;
      }
  ...
  return (
    <ContentContainer>
      <Section>
        <>
          {appointment?._id === undefined && (
            <h1>{t("pages.appointment.titleNew")}</h1>
          )}
          {appointment?._id !== undefined && (
            <h1>{t("pages.appointment.titleEditing")}</h1>
          )}
        </>
        <form
        ...
        >
          ...
          <InputSelect
            ...
            label={t("pages.appointment.tariffs.title")}
            name={"tariff"}
            register={register}
            options={getTariffsOptions()}
          ...
          {customTitle && (
            <InputText
              ...
              label={t("fields.appointmentType")}
              name={"type"}
              autoCapitalize={true}
              required={true}
              type={"text"}
          ...
          <Calendar
            className={"col-span-4 md:col-span-2 md:row-span-3"}
            value={date}
          ...
          <div className={"col-span-4 md:col-span-2"}>
            <div>
              {format(date, "EEEE dd MMMM", { locale: getLocale() })}{" "}
              {t("fields.fromHour")}{" "}
              {format(date, "HH:mm", { locale: getLocale() })}{" "}
              {t("fields.toHour")}{" "}
              {format(add(date, { minutes: duration }), "HH:mm")}
            </div>
          </div>
          ...
          <InputSelect
            className={"col-span-2"}
            label={t("fields.appointmentState")}
            name={"state"}
            required={false}
            register={register}
            error={errors.payment}
