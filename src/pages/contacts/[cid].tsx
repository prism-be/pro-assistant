...
const Contacts: NextPage = () => {
  ...
  const { data: contact, mutate: mutateContact } = useSWR(
    "/data/contacts/" + cid,
    loadContact
  );
  const {
  ...
  const saveContactForm = async (data: any) => {
    if (cid === "000000000000000000000000") {
      data.id = "";
      const result = await postData<InsertResult>(
    ...
    <ContentContainer>
      <Section>
        <div className={"flex"}>
          ...
          <h1>
            {t("pages.contacts.details.title")} {contact?.lastName}{" "}
            {contact?.firstName}
          </h1>
        </div>
        <form
        ...
        >
          ...
          <div
            className={
              "md:col-start-4 pt-4 md:pt-0 md:col-span-3 lg:col-span-3 lg:col-start-10"
            }
          >
            <Button
              text={t("actions.save")}
              onClick={handleSubmit(onSaveContactSubmit)}
              secondary={true}
            />
          </div>
        </form>
