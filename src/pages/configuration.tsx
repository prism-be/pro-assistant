...
const Tariffs = () => {
  ...
  const { data: tariffs, mutate: mutateTariffs } =
    useSWR<Tariff[]>("/data/tariffs/list");
          ...
          <Popup>
            <form>
              <div className={"grid grid-cols-2 gap-2"}>
                ...
                <div className={"col-span-2"}>
                  <InputText
                    label={t("tariffs.defaultDuration")}
                ...
                <div className={"col-span-2"}>
                  <InputColor
                    label={t("tariffs.color")}
                    name={"backgroundColor"}
                    setValue={setValue}
                    error={errors.backgroundColor}
                    initialColor={getValues()["backgroundColor"]}
                  />
                </div>
                <Button
                ...
                <Button
                  ...
                  onClick={handleSubmit(onSaveTariff)}
                />
              </div>
            </form>
          </Popup>
        ...
        <div>
          {tariffs?.map((tariff) => (
            <div key={tariff._id} className={"flex"}>
              ...
              <div className={"pl-2"}>({tariff.price.toFixed(2)} &euro;)</div>
            </div>
          ))}
        </div>
      </>
...
const Documents = () => {
  ...
  const { data: settings, mutate: mutateSettings } = useSWR<Setting[]>(
    "/data/settings/list"
  );
  ...
  useEffect(() => {
    if (settings) {
      ...
      setValue(
        "accentuateColor",
        findSetting("document-header-accentuate-color")?.value
      );
      setAccentuateColor(
        findSetting("document-header-accentuate-color")?.value
      );
  ...
  const saveDocumentHeaders = async () => {
    ...
    settings.push({
      _id: "document-header-accentuate-color",
      value: data.accentuateColor,
    });
    settings.push({ _id: "document-header-logo", value: data.logo });
    settings.push({ _id: "document-header-signature", value: data.signature });
    ...
    <Section>
      <>
        ...
        <div className={"grid grid-cols-4 gap-2"}>
          ...
          <TextArea
            className={"col-span-4 md:col-span-2"}
            label={t("documents.header.address")}
            name={"address"}
            register={register}
          />
          <InputImage
            className={"col-span-4 md:col-span-2"}
            label={t("documents.header.logo")}
            name={"logo"}
            register={register}
            setValue={setValue}
            initialPreview={logo}
          ...
          <InputImage
            className={"col-span-4 md:col-span-2"}
            label={t("documents.header.signature")}
            name={"signature"}
            register={register}
            setValue={setValue}
            initialPreview={signature}
          />
          <InputText
            className={"col-span-4 md:col-span-2"}
            label={t("documents.header.yourName")}
            name={"yourName"}
            type={"text"}
            register={register}
            setValue={setValue}
          ...
          <InputText
            className={"col-span-4 md:col-span-2"}
            label={t("documents.header.yourCity")}
            name={"yourCity"}
            type={"text"}
            register={register}
            setValue={setValue}
          ...
          <InputColor
            className={"col-span-4 md:col-span-2"}
            label={t("documents.header.accentuateColor")}
            name={"accentuateColor"}
            setValue={setValue}
            initialColor={accentuateColor}
          />
        </div>
      </>
    </Section>
