...
import { AlertType, clear, onAlert } from "@/modules/events/alert";
...
export const Alert = ({ id, fade }: Props) => {
  ...
  useEffect(() => {
    ...
    const subscription = onAlert(id).subscribe(
      (alert: { message: any; itemId: number; autoClose: any }) => {
        if (!alert.message) {
          setAlerts([]);
        } else {
    ...
    <div className={"fixed top-20 left-0 right-0 p-4"}>
      {alerts.map((alert, index) => (
        <div
          key={index}
          className={
            "m-auto max-w-screen-md bg-white p-4 border rounded shadow relative mb-2 " +
            cssClasses(alert)
          }
        >
          <a
            className={
              "w-6 cursor-pointer block absolute top-2 right-2 " +
              cssClasses(alert)
            }
