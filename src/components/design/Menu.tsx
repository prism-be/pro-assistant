...
import {
  onToggledMobileMenu,
  toggledMobileMenu,
} from "@/modules/events/mobileMenu";
...
const Menu = () => {
  ...
  const itemClassName =
    "block px-4 py-2 hover:bg-gray-100 no-underline hover:no-underline";

  return (
    <div
      className={
        "bg-white w-64 border-r h-100 fixed top-14 h-full " +
        (displayMobileMenu ? "block" : "hidden md:block")
      }
      ...
      <ul>
        ...
        <li className={getActiveLinkClass("/agenda")}>
          <Link
            ...
            href={"/agenda/" + format(new Date(), "yyyy-MM-dd")}
          >
            {t("menu.agenda")}
          </Link>
        </li>
