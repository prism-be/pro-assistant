...
async function getAppointment(db: Db, appointmentId: string) {
  const appointment = await db
    ...
    .findOne<Appointment>({ _id: new ObjectId(appointmentId) });
...
async function getContact(db: Db, contactId: any) {
  const contact = await db
    .collection("contacts")
    .findOne<Contact>({ _id: new ObjectId(contactId) });
...
async function resizeImage(image: string, w: number, h: number) {
  return await sharp(
    Buffer.from(Buffer.from(image.split(";base64,").pop() as string, "base64"))
  )
...
async function writeHeader(db: Db, doc: jsPDF) {
  const accentuateColor = hexToRgb(
    await getSetting(db, "document-header-accentuate-color")
  );
...
async function writeAddress(db: Db, doc: jsPDF, contact: Contact) {
  ...
  contactAddress +=
    `${contact.street ?? ""} ${contact.number ?? ""}`.trim() + "\n";
  contactAddress +=
    `${contact.zipCode ?? ""} ${contact.city ?? ""}`.trim() + "\n";
...
async function writeContent(doc: jsPDF, title: string, content: string) {
  ...
  doc.text(
    doc.splitTextToSize(
      content,
      doc.internal.pageSize.width - documentMargin * 2
    ),
    documentMargin,
    11
  );
...
async function writeFooter(db: Db, doc: jsPDF) {
  ...
  doc.text(
    signatureContent,
    doc.internal.pageSize.width - documentMargin - signatureWidth,
...
async function saveDocument(
  ...
  let fileName = `${format(new Date(), "yyyy-MM-dd HH:mm")}- ${
    appointment.lastName
  } ${appointment.firstName} - ${title}.pdf`;
  ...
  const existing = await db
    .collection("appointments")
    .findOne<Appointment>({ _id: new ObjectId(appointment._id) });
  if (!existing) {
    throw new Error("Appointment not found");
  }
  ...
  await db
    .collection("appointments")
    .updateOne({ _id: new ObjectId(appointment._id) }, { $set: existing });
}
...
async function replaceContent(
  ...
  const data = {
    ...
    appointmentDate: format(parseISO(appointment.startDate), "dd/MM/yyyy", {
      locale: getLocale(),
    }),
    appointmentHour: format(parseISO(appointment.startDate), "HH:mm", {
      locale: getLocale(),
    }),
    paymentDate: format(
      parseISO(appointment.paymentDate ?? appointment.startDate),
      "dd/MM/yyyy",
      { locale: getLocale() }
    ),
  ...
  const formattedContent = Mustache.render(
...
export async function generateDocument<TResult>(
  db: Db,
  request: DocumentRequest
): Promise<void> {
  const { title, body } = await getDocumentConfiguration(
    db,
    request.documentId
  );
  const appointment = await getAppointment(db, request.appointmentId);
  const contact = await getContact(db, appointment.contactId);
  const { formattedTitle, formattedContent } = await replaceContent(
    db,
    title ?? "",
    body ?? "",
    appointment,
...
export async function deleteDocument<TResult>(
  db: Db,
  request: DocumentRequest
): Promise<void> {
  const bucket = new GridFSBucket(db);
