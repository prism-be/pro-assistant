    ...
    public async Task<List<Contact>> Handle(SearchContacts request, CancellationToken cancellationToken)
    {
        ...
        if (!string.IsNullOrWhiteSpace(request.LastName))
        {
            filters.Add(Builders<Contact>.Filter.Regex(x => x.LastName,
                BsonRegularExpression.Create(new Regex($"^{request.LastName}", RegexOptions.IgnoreCase, TimeSpan.FromMilliseconds(100)))));
        }

        if (!string.IsNullOrWhiteSpace(request.FirstName))
        {
            filters.Add(Builders<Contact>.Filter.Regex(x => x.FirstName,
                BsonRegularExpression.Create(new Regex($"^{request.FirstName}", RegexOptions.IgnoreCase, TimeSpan.FromMilliseconds(100)))));
        }

        if (!string.IsNullOrWhiteSpace(request.PhoneNumber))
        {
            filters.Add(Builders<Contact>.Filter.Regex(x => x.PhoneNumber,
                BsonRegularExpression.Create(new Regex($"^{request.PhoneNumber}", RegexOptions.IgnoreCase, TimeSpan.FromMilliseconds(100)))));
        }

        if (!string.IsNullOrWhiteSpace(request.BirthDate))
        {
            filters.Add(Builders<Contact>.Filter.Regex(x => x.BirthDate,
                BsonRegularExpression.Create(new Regex($"^{request.BirthDate}", RegexOptions.IgnoreCase, TimeSpan.FromMilliseconds(100)))));
        }
