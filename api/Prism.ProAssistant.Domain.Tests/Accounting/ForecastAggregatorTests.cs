...
public class ForecastAggregatorTests
{
    ...
    public async Task ComputeBudget_WorkDaily()
    {
        ...
        var dailyPrevisionExpense = new ForecastPrevision
        {
            ...
            Type = ForecastPrevisionType.Expense,
            RecurringCount = 1,
            RecurringType = RecurringType.WorkDaily,
            Name = "Daily from 5",
            StartDate = new DateTime(2023, 1, 5),
            EndDate = new DateTime(2023, 5, 14)
        };
        ...
        aggregator.State.Should().NotBeNull();
        aggregator.State!.WeeklyBudgets.Should().HaveCount(53);
        aggregator.State!.WeeklyBudgets[0].Amount.Should().Be(0);
        aggregator.State!.WeeklyBudgets[1].Amount.Should().Be(5 * 50 - 2 * 10);
        aggregator.State!.WeeklyBudgets[2].Amount.Should().Be(5 * 50 - 5 * 10);
        aggregator.State!.WeeklyBudgets.Sum(x => x.Amount).Should().Be(95 * 50 - 92 * 10);
    }
